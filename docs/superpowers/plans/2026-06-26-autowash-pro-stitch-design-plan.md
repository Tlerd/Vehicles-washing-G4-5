...
* **Phát triển Back-end (Phat \& Binh \& DAnh)**:
  ...
  * `\\\\\\\\\\\\\\\[ ]` Database Schema \& Entity mapping (bảng `customers` và `otp\\\\\\\\\\\\\\\_tokens`): **Phat** (Code chính), **Binh** (Review) \& **DAnh** (Hỗ trợ)
...
* **Phát triển Back-end (Binh)**:
  ...
  * `\\\\\\\\\\\\\\\[ ]` Cài đặt công thức tính điểm $P = (V / 1000) \* K\_h \* K\_{km}$ trong tầng Service: **Binh** (Code chính)
...
* **Phát triển Back-end (DAnh)**:
  ...
  * `\\\\\\\\\\\\\\\[ ]` REST API cập nhật vòng đời trạng thái của lịch đặt (`PENDING` -> `CONFIRMED` -> `CHECKED\\\\\\\\\\\\\\\_IN` -> `COMPLETED`) kèm kiểm tra quyền vai trò: **DAnh** (Code chính)
...
* **Phát triển Back-end (DAnh \& Phat)**:
  ...
  * `\\\\\\\\\\\\\\\[ ]` Tích hợp mock-AI phân tích sinh nội dung ưu đãi và tự động lưu vào bảng `promotions` với hệ số $K\_{km}$: **DAnh** (Code chính) \& **Phat** (Review/Hỗ trợ)
