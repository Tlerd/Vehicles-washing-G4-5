...
* **Then** the client converts phone to `+84901234567` and posts to `/api/auth/login`.
...
* **Front-end Development (Phong & An)**:
  * `[ ]` Design Member Login Form & Quick Booking Form for Guest Users: **Phong** (Lead) & **An** (Support/Review)
  * `[ ]` Handle JWT token storage in LocalStorage and configure Axios Interceptor to automatically attach token to headers: **An** (Lead) & **Phong** (Support/Review)
* **Back-end Development (Phat & Binh & Anh)**:
  * `[ ]` Configure Spring Security & JWT Token Provider for code generation: **Binh** (Lead), **Phat** (Review) & **Anh** (Support)
  * `[ ]` Write authentication service to verify login credentials and encrypt BCrypt password: **Binh** (Lead), **Phat** (Review) & **Anh** (Support)
  * `[ ]` Implement API `/api/v1/auth/login` returning token: **Phat** (Lead), **Binh** (Review) & **Anh** (Support)
