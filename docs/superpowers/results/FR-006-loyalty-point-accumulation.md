...
* **Front-end Development (An & Nguyen)**:
  * `[ ]` Display estimated accumulated points in Cart Sidebar and Confirmation screen: **An** (Lead) & **Nguyen** (Support/Review)
* **Back-end Development (Phat & Binh)**:
  * `[ ]` Implement points calculation formula $P = (V / 1000) * K_h * K_{km}$ in Service layer: **Binh** (Lead) & **Phat** (Support/Review)
  * `[ ]` Function to add accumulated points to customer account when booking status changes to `COMPLETED`: **Binh** (Lead) & **Phat** (Support/Review)
