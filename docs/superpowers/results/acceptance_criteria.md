...
### AC-101: Twilio OTP Verification during Registration
*   **Scenario 1: OTP Sent successfully**
    ...
    *   **And** the proxy backend must call the Twilio Verify API and return success.
*   **Scenario 2: Successful verification**
    ...
    *   **Then** the verify button must show a success indicator, set `isOtpVerified = true`, and enable the "Sign Up & Start" submission button.
*   **Scenario 3: Failed verification**
    *   **Given** the user enters an incorrect 6-digit code.
    *   **When** the user clicks "Verify".
    *   **Then** the system must display a red error message: "Incorrect OTP code. Please try again," and keep the sign-up submission button disabled.
