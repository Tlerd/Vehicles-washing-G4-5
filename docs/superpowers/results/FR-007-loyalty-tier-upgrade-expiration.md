...
* **Front-end Development (Nguyen & Phong)**:
  * `[ ]` UI to display tier upgrade progress (Silver, Gold, Diamond) and alert for expiring points: **Nguyen** (Lead) & **Phong** (Support/Review)
* **Back-end Development (Phat & Binh)**:
  * `[ ]` Algorithm to update customer tier based on actual spending in the past 12 months: **Phat** (Lead) & **Binh** (Support/Review)
  * `[ ]` Scheduler to run automatically monthly, scan and retrieve expired points / downgrade tier (Month Roll): **Phat** (Lead) & **Binh** (Support/Review)
