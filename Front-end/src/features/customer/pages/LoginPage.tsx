...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (result.success && result.role) {
      onLoginSuccess();
      navigate(getDestinationForRole(result.role), { replace: true });
    } else {
        ...
        <section className={styles.card} aria-labelledby="portal-access-title">
            ...
            <form className={styles.form} onSubmit={handleLogin}>
              <fieldset className={styles.fieldset}>
                ...
                <input
                  ...
                  inputMode="tel"
                  autoComplete="tel"
                  placeholder={selectedRoleOption.demoPhone || '0901234567'}
                  value={loginPhone}
              ...
              <dl className={styles.quickInfoGrid}>
                ...
                <div className={styles.quickInfoCard}>
                  <dt>Demo number</dt>
                  <dd>{selectedRoleOption.demoPhone || 'Registered account'}</dd>
                </div>
                <div className={styles.quickInfoCard}>
                  <dt>Destination</dt>
                ...
                <div key={option.id} className={styles.demoItem}>
                  ...
                  <dd>
                    {option.demoPhone
                      ...
                      : 'Use a registered customer account'}
                  </dd>
                </div>
              ))}
            </dl>
          ...
          {selectedRole === 'CUSTOMER' && activeTab !== 'verify' && (
            <footer className={styles.cardFooter}>
              <p className={styles.footerNote}>Guest booking needs a backend guest-session API that is not available yet.</p>
              <button className={styles.guestBtn} type="button" disabled title="Guest API is not available">
