    ...
    <section id="locations" className="py-24 bg-surface-soft relative">
      <div className="max-w-[1400px] mx-auto px-5 md:px-10 lg:px-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {LOCATIONS.map((loc) => (
            <div key={loc.id} className="bg-surface rounded-3xl overflow-hidden border border-border shadow-sm hover:shadow-md transition-all group flex flex-col">
              ...
              <div className="h-48 w-full bg-primary-light/30 relative overflow-hidden flex items-center justify-center">
                ...
                <div className="absolute w-12 h-12 bg-surface rounded-full shadow-lg flex items-center justify-center text-primary z-10 group-hover:scale-110 transition-transform">
                  <MapPin className="w-6 h-6" />
                </div>
                {/* Decorative map elements */}
                <div className="absolute top-1/2 left-1/4 w-32 h-2 bg-surface/60 rounded-full -rotate-45 blur-[1px]"></div>
                <div className="absolute top-1/3 right-1/4 w-40 h-3 bg-surface/60 rounded-full rotate-12 blur-[1px]"></div>
              </div>

              <div className="p-8 flex flex-col flex-grow">
                <h3 className="text-2xl font-display font-bold text-text-primary mb-4">{t(`locations.items.${loc.id}.name`)}</h3>

                <div className="space-y-4 mb-8 flex-grow">
                  <div className="flex items-start gap-3 text-text-secondary">
                    <MapPin className="w-5 h-5 text-primary shrink-0 mt-0.5" />
                    <span className="leading-relaxed">{t(`locations.items.${loc.id}.address`)}</span>
                  </div>
                  <div className="flex items-start gap-3 text-text-secondary">
                    <Clock className="w-5 h-5 text-primary shrink-0 mt-0.5" />
                    <span>{t('locations.openDaily', { hours: loc.hours })}</span>
                  </div>
                  <div className="flex items-start gap-3 text-text-secondary">
                    ...
                    <span>{t('locations.serviceBays')}</span>
                  </div>
                </div>
                ...
                >
                  {t('locations.selectButton')} <ArrowRight className="w-4 h-4" />
                </button>
              </div>
            </div>
          ))}
